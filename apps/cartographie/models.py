...
class Cimetiere(models.Model):
    ...
    @property
    def superficie_zones_totale_m2(self):
        """
        Somme des superficies de TOUTES les zones déjà créées (tous types
        confondus). Sert à vérifier qu'on ne dépasse jamais la superficie
        totale du cimetière lors de la création d'une nouvelle zone.
        """
        from django.db.models import Sum
        total = self.zones.aggregate(total=Sum("superficie_m2"))["total"]
        return float(total) if total else 0.0
...
class Zone(models.Model):
    ...
    @property
    def capacite_theorique_max(self):
        """
        Nombre maximum de tombeaux que cette zone peut physiquement
        accueillir, compte tenu de sa superficie et de la taille standard
        du tombeau définie sur le cimetière (cahier des charges §2.2).
        Seules les zones EXPLOITABLE ont une capacité (les chemins, zones
        techniques/administratives et non-exploitables n'accueillent aucun
        caveau).
        """
        if self.type_zone != Zone.TypeZone.EXPLOITABLE:
            return 0
        surface_tombeau = self.cimetiere.surface_tombeau_m2
        if surface_tombeau <= 0:
            return 0
        return int(float(self.superficie_m2) / surface_tombeau)

    @property
    def capacite_deja_allouee(self):
        """Somme des capacités théoriques (rangées × colonnes) des blocs déjà créés dans cette zone."""
        from django.db.models import F, Sum
        total = self.blocs.aggregate(
            total=Sum(F("nombre_rangees") * F("nombre_colonnes"))
        )["total"]
