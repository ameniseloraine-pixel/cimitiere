...
def DashboardView(page: ft.Page, client):
    ...
    def jauge_saturation(taux: float) -> ft.Container:
        ...
        return ft.Container(
            ...
            bgcolor=ft.colors.SURFACE, padding=16, border_radius=12, border=ft.border.all(1, "#e5e7eb"),
        )
    ...
    def tableau_blocs(par_bloc: list) -> ft.Container:
        ...
        # Le DataTable a une largeur intrinsèque fixe : sur mobile on l'enveloppe
        # dans une Row scrollable horizontalement pour éviter tout débordement
        # ou compression illisible des colonnes.
        table = ft.DataTable(
            columns=[
    ...
    def charger():
        ...
        try:
            ...
            # col={"xs": 6, ...} : sur très petit écran on garde 2 cartes par ligne
            # plutôt que de les empiler une par une, ce qui est plus lisible
            # tout en restant compact.
            cartes_stats = ft.ResponsiveRow([
                ft.Container(carte_stat("Caveaux totaux", occ["total"], ft.icons.GRID_VIEW, COULEUR_PRIMAIRE), col={"xs": 6, "sm": 6, "md": 3}),
                ft.Container(carte_stat("Disponibles", occ["disponibles"], ft.icons.CHECK_CIRCLE, "#22c55e"), col={"xs": 6, "sm": 6, "md": 3}),
                ft.Container(carte_stat("Réservés", occ["reserves"], ft.icons.HOURGLASS_EMPTY, "#f97316"), col={"xs": 6, "sm": 6, "md": 3}),
                ft.Container(carte_stat("Occupés", occ["occupes"], ft.icons.LOCK, "#ef4444"), col={"xs": 6, "sm": 6, "md": 3}),
            ...
            if fin:
                sections.append(ft.ResponsiveRow([
                    ft.Container(carte_stat("Revenus totaux", f"{fin['revenus_total']:,.0f} FCFA", ft.icons.PAYMENTS, COULEUR_PRIMAIRE), col={"xs": 12, "sm": 6, "md": 4}),
                    ft.Container(carte_stat("Revenus ce mois", f"{fin['revenus_ce_mois']:,.0f} FCFA", ft.icons.CALENDAR_MONTH, "#22c55e"), col={"xs": 12, "sm": 6, "md": 4}),
                    ft.Container(carte_stat("Factures en retard", fin["factures_en_retard"], ft.icons.WARNING_AMBER, "#ef4444"), col={"xs": 12, "sm": 12, "md": 4}),
                ], spacing=12, run_spacing=12))
    ...
    return ft.Container(
        content=ft.Column([
            ft.Row([
                ft.Text("Tableau de bord", size=18 if est_mobile else 20, weight=ft.FontWeight.BOLD),
